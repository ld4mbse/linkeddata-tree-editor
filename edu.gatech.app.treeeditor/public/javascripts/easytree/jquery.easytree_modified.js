...
(function ($) {
    ...
    var EasyTree = function (jQueryContext, options) {
        ...
        function drag(event) {
            ...
            /**********  Modification *************/
            /**************************************/
            var sBlock = _dnd.sourceNode.block; 
            var tBlock = _dnd.targetNode.block; 
            var foundCircularProblem = -1;

            if( sBlock.name === tBlock.name ) foundCircularProblem = 1;
            
            for (var i = 0; i < tBlock.childBlocks.length; i++) {
                if(sBlock.name === tBlock.childBlocks[i].name)
                    foundCircularProblem = 2;

                for (var j = 0; j < tBlock.childBlocks[i].childBlocks.length; j++) {
                    if(sBlock.name === tBlock.childBlocks[i].childBlocks[j].name)
                        foundCircularProblem = 2    ;

                    for (var k = 0; k < tBlock.childBlocks[i].childBlocks[j].childBlocks.length; k++) {
                        if(sBlock.name === tBlock.childBlocks[i].childBlocks[j].childBlocks[k].name)
                            foundCircularProblem = 2;
                    };
                };
            };

            if(tBlock.parentBlocks.length === 0) foundCircularProblem = -2;

            log('foundCircularProblem: ' + foundCircularProblem);

            if(foundCircularProblem > 0){
        ...
        function buildTree(nodes) {
            ...
            if (_settings.enableDnd) {
                ...
                $(document).on("mouseover", dragOver);
                $(document).on("mouseout", dragLeave);
            }
